...
func (cm *simpleContextManager) WaitForCompletion(ctx context.Context, duration time.Duration) error {
	// Wait until either:
	// - the caller's context is cancelled (return ctx.Err()), or
	// - the duration elapses (return nil).
	//
	// Important: do NOT wrap ctx with a timeout context here, because that would
	// turn a successful wait into context.DeadlineExceeded.
...
func ProcessItems(ctx context.Context, items []string) ([]string, error) {
	...
	for _, item := range items {
		// If already cancelled, return what we have so far.
