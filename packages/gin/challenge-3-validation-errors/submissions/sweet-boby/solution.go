...
func validateProduct(product *Product) []ValidationError {
	...
	if ok := isValidSKU(product.SKU); !ok {
	...
	if ok := isValidCurrency(product.Currency); !ok {
		errors = append(errors, ValidationError{
			Field: "currency",
	...
	if ok := isValidCategory(product.Category.Name); !ok {
		errors = append(errors, ValidationError{
			Field: "category",
	...
	if ok := isValidSlug(product.Category.Slug); !ok {
		errors = append(errors, ValidationError{
			Field: "slug",
	...
	if ok := isValidWarehouseCode(product.Inventory.Location); !ok {
		errors = append(errors, ValidationError{
			Field: "warehouse",
...
func sanitizeProduct(product *Product) {
	...
	product.Description = strings.Trim(product.Description, " ")
...
func createProduct(c *gin.Context) {
	...
	fmt.Println("len(validationErrors): ",len(validationErrors) )
		fmt.Printf("validationErrors: %+v ",validationErrors )
...
func validateSKUEndpoint(c *gin.Context) {
	...
	if ok := isValidSKU(request.SKU); !ok {
		c.JSON(200, APIResponse{
			Success: false,
		})
		return
