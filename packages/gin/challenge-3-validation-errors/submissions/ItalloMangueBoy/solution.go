...
// Regex patterns for validation (to be used in validation functions)
var skuRegex = regexp.MustCompile(`^[A-Z]{3}-\d{3}-[A-Z]{3}$`)
var slugRegex = regexp.MustCompile(`^[a-z0-9]+(?:-[a-z0-9]+)*$`)
var warehouseRegex = regexp.MustCompile(`^WH\d{3}$`)
...
func isValidCategory(categoryName string) bool {
	...
	for _, registredCategory := range categories {
		if categoryName == registredCategory.Name {
			return true
		}
	}
...
func isValidWarehouseCode(code string) bool {
	...
	if warehouseRegex.MatchString(code) {
		if slices.Contains(validWarehouses, code) {
			return true
		}
	}
	...
	for _, existingProduct := range products {
		if !isValidSKU(product.SKU) {
			errors = append(errors, ValidationError{
				Field:   "sku",
				Value:   product.SKU,
				Message: "Invalid SKU format. Expected format: ABC-123-XYZ",
				Tag:     "format",
			})
			break
		}
