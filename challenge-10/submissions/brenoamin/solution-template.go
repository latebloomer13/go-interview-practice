...
func NewTriangle(a, b, c float64) (*Triangle, error) {
	if a <= 0 || b <= 0 || c <= 0 {
		return nil, errors.New("a + b + c must be greater than zero")
	}
...
func (t *Triangle) Area() float64 {
	sP := (t.SideA + t.SideB + t.SideC) / 2.0
	a := math.Sqrt(sP * (sP - t.SideA) * (sP - t.SideB) * (sP - t.SideC))
	return a
}
...
func (t *Triangle) String() string {
	return fmt.Sprintf("Triangle: sides -> %v, %v and %v ", t.SideA, t.SideB, t.SideC)
}
...
func (sc *ShapeCalculator) LargestShape(shapes []Shape) Shape {
	...
	var lShape Shape
	for _, shape := range shapes {
		if shape.Area() > lShapeArea {
			lShapeArea = shape.Area()
			lShape = shape
		}
	}
	return lShape
