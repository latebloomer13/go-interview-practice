...
const (
	RectangleError          = "width and height must be positive"
	CircleError             = "radius must be positive"
	TriangleError           = "sides must be positive"
	TriangleInequalityError = "sides do not satisfy the triangle inequality"
)
...
func NewRectangle(width, height float64) (*Rectangle, error) {
	if width <= 0 || height <= 0 {
		return nil, errors.New(RectangleError)
	}
...
func NewCircle(radius float64) (*Circle, error) {
	if radius <= 0 {
		return nil, errors.New(CircleError)
	}
...
func NewTriangle(a, b, c float64) (*Triangle, error) {
	if a <= 0 || b <= 0 || c <= 0 {
		return nil, errors.New(TriangleError)
	}

	if a+b <= c || a+c <= b || b+c <= a {
		return nil, errors.New(TriangleInequalityError)
	}
