...
func NewRectangle(width, height float64) (*Rectangle, error) {
	if width <= 0 || height <= 0 {
		return nil, errors.New("Rectangle sides must be greater than 0")
	}
	return &Rectangle{
		Width:  width,
		Height: height,
...
func (r *Rectangle) String() string {
	return fmt.Sprintf("Rectangle %v width x %v height", r.Width, r.Height)
}
...
func NewCircle(radius float64) (*Circle, error) {
	if radius <= 0 {
		return nil, errors.New("Circle radius must be greater than 0")
	}
	return &Circle{
		Radius: radius,
	}, nil
...
func NewTriangle(a, b, c float64) (*Triangle, error) {
	if a <= 0 || b <= 0 || c <= 0 {
		return nil, errors.New("Triangle sides must be greater than 0")
	}
	if a+b <= c || a+c <= b || b+c <= a {
		return nil, errors.New("Sum of two sides must be greater than the third side")
	}
	return &Triangle{
		SideA: a,
		SideB: b,
...
func (t *Triangle) Area() float64 {
	s := t.Perimeter() / 2.0
	sum := s * (s - t.SideA) * (s - t.SideB) * (s - t.SideC)
...
func (sc *ShapeCalculator) SortByArea(shapes []Shape, ascending bool) []Shape {
	...
	sort.Slice(cloned, func(a, b int) bool {
		if ascending {
			return cloned[a].Area() < cloned[b].Area()
		}
		return cloned[a].Area() > cloned[b].Area()
	})
	return cloned
